...
{
	/*
	 * needItem	onlyDestroyUpwards	enableEnchantmentMode	works with Axe(A) EnchantedAxe (E) Creative (no hand)(C) non-item(N)
	 * true		true				true							NONE
	 * true		true				false					AE
	 * true		false				true							NONE
	 * true		false				false					AE
	 * false	true				true					AECN	ALL
	 * false	true				false					AECN	ALL
	 * false	false				true					AECN	ALL
	 * false	false				false					AECN	ALL
	 * 
	 * Conclusion: 
	 * needItem==false always works
	 * needItem==true deactivates non-item treecapitator effect; still works with axes (good!)
	 * ## needItem==true && enableEnchantmentMode==true breaks axe functionality ##
	 */
    ...
    {
        ItemStack axe = entityPlayer.getHeldItem(EnumHand.MAIN_HAND);//.getCurrentEquippedItem();
    ...
    {
        axe = player.getHeldItem(EnumHand.MAIN_HAND);//.getCurrentEquippedItem();
    ...
    {
        ItemStack item = player.getHeldItem(EnumHand.MAIN_HAND);//.getCurrentEquippedItem();
        // Below two lines by jco2641
        ...
        if (TCSettings.enableEnchantmentMode)
        {
            if ((item != null) && item.isItemEnchanted())
                for (int i = 0; i < item.getEnchantmentTagList().tagCount(); i++)
                {
                    ...
                    if (tag.getShort("id") == enchantmentID)//(tag.getShort("id") == TCSettings.treecapitating.effectId)
                    {
                        axe = item;
                        return true;
                    }
    ...
    {
        ItemStack item = entityPlayer.getHeldItem(EnumHand.MAIN_HAND);//.getCurrentEquippedItem();
        // Below two lines by jco2641
        Enchantment enchantment = Enchantment.getEnchantmentByLocation("treecapitating");
        int enchantmentID = Enchantment.getEnchantmentID(enchantment);
        if (TCSettings.enableEnchantmentMode)
        ...
        else
        {
            if ((item != null) && !ToolRegistry.instance().isAxe(item) && TCSettings.allowAutoAxeDetection)
                ToolRegistry.autoDetectAxe(entityPlayer.world, pos, item); //from worldObj --AstroTibs
    ...
    public void doProceduralChop(BlockPos pos)
    {
        ...
        if ((currentAxeDamage > 0.0F) && (axe != null))
        {
            ...
            for (int i = 0; i < MathHelper.floor(currentAxeDamage); i++) //floor_double()
                axe.getItem().onBlockDestroyed(axe, world, world.getBlockState(pos), pos, player);//.onBlockDestroyed(axe, world, treeDef.getLogList().get(0).getBlock(), pos, player);
        }
        ...
        if ((currentShearsDamage > 0.0F) && (shears != null))
        {
            ...
            for (int i = 0; i < Math.floor(currentShearsDamage); i++)
                if (shears.getItem().equals(Items.SHEARS))
                    ...
                    shears.getItem().onBlockDestroyed(shears, world, world.getBlockState(pos), pos, player);//.onBlockDestroyed(shears, world, treeDef.getLeafList().get(0).getBlock(), pos, player);
        }
        ...
        if (TCSettings.stackDrops)
            while (drops.size() > 0)
            	/*
            	 * I think all spawnEntityInWorld has been simply replaced with spawnEntity? idk --AstroTibs
            	 */
                ...
                for (int z = -range; z <= range; z++)
                    if ((x != 0) || (y != 0) || (z != 0))
                    {
                        ...
                        if (!world.isAirBlock(pos2))
                        {
                            ...
                            if (block.isLeaves(world.getBlockState(pos), world, pos2))//(block.isLeaves(world, pos2))
                            {
                                if (shouldLog)
            ...
            for (int y = -1; y <= range; y++)
                for (int z = -range; z <= range; z++)
                    if ((x != 0) || (y != 0) || (z != 0))
                    {
                        ...
                        /*
                         * Essentially with the below, I'm trying to capture the functionality
                         * of the isValid boolean, which returned a "true" when blockID.getBlock() was not null.
                         * I don't know what bskprs was doing with all this stuff but whatever.
                         * --AstroTibs
                         */
                        if (!blockID.id.equals("")) //(blockID.isValid())
    ...
    private int shearsHotbarIndex(EntityPlayer entityPlayer)
    {
        for (int i = 0; i < 9; i++)
        {
            ItemStack item = entityPlayer.inventory.mainInventory.get(i);//.inventory.mainInventory[i];
    ...
    private void destroyBlocksWithChance(World world, List<BlockPos> list, float f, boolean canShear)
    {
        while (list.size() > 0)
        {
            ...
            if (!block.isAir(world.getBlockState(pos), world, pos))//(!block.isAir(world, pos))
            {
                        ...
                        && !(player.capabilities.isCreativeMode && TCSettings.disableCreativeDrops))
                {
                    ...
                    {
                        List<ItemStack> drops = target.onSheared(shears, player.world, pos, //from worldObj --AstroTibs
                                EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, shears));//(Enchantment.fortune.effectId, shears));
                ...
                else if (!(player.capabilities.isCreativeMode && TCSettings.disableCreativeDrops))
                {
                    if (TCSettings.stackDrops)
                        ...
                        block.dropBlockAsItem(world, pos, world.getBlockState(pos), EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, player.getHeldItemMainhand()));//.getFortuneModifier(player));
                    else
                        block.dropBlockAsItem(world, startPos, world.getBlockState(pos), EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, player.getHeldItemMainhand()));//.getFortuneModifier(player));
                ...
                /*
                 * The line below prevents chopping down larger trees AND de-syncs with the server.
                 * I don't know what it does so I'm commenting it out, sadly.
                 * --AstroTibs
                 */
                //player.addStat(StatList.MINE_BLOCK_STATS.get(Block.getIdFromBlock(block)), 1);//.mineBlockStatArray[Block.getIdFromBlock(block)], 1);
    ...
    private void addDrop(Block block, int metadata, BlockPos pos)
    {
        ...
        if ((block.canSilkHarvest(world, pos, state, player) && (EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, player.getHeldItemMainhand()))> 0))//EnchantmentHelper.getSilkTouchModifier(player)))
        {
        ...
        {
            stacks = block.getDrops(world, pos, state, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, player.getHeldItemMainhand()));//.getFortuneModifier(player));
        }

        addDrops(stacks);
    }

    private void addDrops(List<ItemStack> stacks) // Had to annotate this to figure out what it's doing --AstroTibs
    {
        if (stacks == null) // If nothing
            return; // return nothing
        for (ItemStack drop : stacks) // For each ItemStack that was dropped
        {
            if (drop == null) // If there's no drop
                continue; // go to the next ItemStack

            int index = -1; // Initialize this as "no index by default (-1)"
            for (int i = 0; i < drops.size(); i++) // Try incremental i value for each and every element in the List<ItemStack>
            {
                if (drops.get(i).isItemEqual(drop)) // If we've matched your List<ItemStack> with the master List<ItemStack>
                {
                    index = i; // Change index from -1 (default) to i (index of the master List<ItemStack>)
                    break; // And we're finished!
                }
            }
            
            // After all of the above is done, you now have a value for index
            
            if (index == -1) // I believe -1 means "not in this list"
            {
                drops.add(drop); // It adds "drop" as a new member of the "drops" List<ItemStack>
                index = drops.indexOf(drop); // Then retrieves the index of that new member
            }
            else // The item "drop" was found in the "drops" list
            {
                int quantity = drop.getCount(); // This is the number of things in the "drop" ItemStack
                drop = drops.get(index); // This is the ItemStack "drop" within the list "drops" that corresponds to the found "index"
                drop.setCount(drop.getCount()+quantity);//.stackSize += quantity; THANKS BETH--THIS CODE BLOWS LMAO
            }

            if (drop.getCount() >= drop.getMaxStackSize())
            {
                int i = drop.getCount() - drop.getMaxStackSize();
                drop.setCount(drop.getMaxStackSize());//.stackSize = drop.getMaxStackSize();
                world.spawnEntity(new EntityItem(world, dropPos.getX(), dropPos.getY(), dropPos.getZ(), drop));
                if (i > 0)
                    drop.setCount(i);//.stackSize = i;
                else
                    drops.remove(index);
            }
        }
    ...
    private boolean damageAxeAndContinue(World world, Block block, BlockPos pos)
    {
        if (axe != null)
        {
            ...
            for (int i = 0; i < (int) Math.floor(currentAxeDamage); i++)
                axe.getItem().onBlockDestroyed(axe, world, world.getBlockState(pos), pos, player);//.onBlockDestroyed(axe, world, block, pos, player);

            currentAxeDamage -= Math.floor(currentAxeDamage);
            
            // I'm hoping the below does the job
            int remainingAxeDurability = player.getHeldItemMainhand().getItemDamage();//.getActiveItemStack().getItemDamage();
            //player.addChatComponentMessage(new TextComponentString( "remainingAxeDurability="+remainingAxeDurability ) );
            if ((axe != null) && (axe.getCount() < 1)) {
            	//player.addChatComponentMessage(new TextComponentString( "remainingAxeDurability="+remainingAxeDurability ) );
                player.getHeldItemMainhand().damageItem(remainingAxeDurability, player);;//.getActiveItemStack().item//.damageItem(remainingAxeDurability, player);//.destroyCurrentEquippedItem();
            	//world.playSound(player, new BlockPos(player.posX, player.posY, player.posZ), SoundEvents.ENTITY_ITEM_BREAK, SoundCategory.BLOCKS, 1F, 1F);
            }
        ...
        if (shears != null)
        {
            ...
            for (int i = 0; i < Math.floor(currentShearsDamage); i++)
                ...
                if (shears.getItem().equals(Items.SHEARS))
                	...
                	//Now need IBlockState instead of just "block" --AstroTibs
                    shears.getItem().onBlockDestroyed(shears, world, world.getBlockState(pos), pos, player);//.onBlockDestroyed(shears, world, block, pos, player);
