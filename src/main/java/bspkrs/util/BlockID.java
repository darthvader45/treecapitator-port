...
public class BlockID
{
    ...
    public BlockID(Block block, int metadata)
    {
        ...
        this(block.getUnlocalizedName(), metadata); // I'm hoping this will do the trick :C --AstroTibs
    }

    public BlockID(Block block)
    {
        //this(GameData.getBlockRegistry().getNameForObject(block).toString(), -1);
        this(block.getUnlocalizedName(), -1); // I'm hoping this will do the trick :C --AstroTibs
    }
    ...
    /*
     * The only call to these is at Treecapitator.hasXLeavesInDist
     * where it says if (blockID.isValid()) then the chain kicks off and the tree comes down.
     * I've blanked out the below functions and replaced the IF condition with my own.
     * --AstroTibs
     */
    /*
