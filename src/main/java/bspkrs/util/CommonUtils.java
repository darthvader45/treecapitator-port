...
public final class CommonUtils
{
    ...
    public void playAtPitch(int i, World world, EntityPlayer entityplayer)
    {
        ...
        //world.playSoundAtEntity(entityplayer, "note.pling", 0.5F, f); //Again, I hope this works --AstroTibs
        world.playSound(entityplayer, new BlockPos(entityplayer.posX, entityplayer.posY, entityplayer.posZ), SoundEvents.BLOCK_NOTE_PLING, SoundCategory.PLAYERS, 0.5F, f);
    }
    ...
    public static String getMinecraftDir()
    {
        ...
        {
        	MinecraftServer worldIn = FMLCommonHandler.instance().getMinecraftServerInstance();
        	return worldIn.getServer().getFile("").getAbsolutePath();
        	//Replaced the below with the above --AstroTibs
    ...
    public static RayTraceResult getPlayerLookingSpot(EntityPlayer player, boolean restrict)//MovingObjectPosition getPlayerLookingSpot(EntityPlayer player, boolean restrict)
    {
        float scale = 1.0F;
        ...
        {
            distance = ((EntityPlayerMP) player).interactionManager.getBlockReachDistance();//.theItemInWorldManager.getBlockReachDistance();
        }
        Vec3d vector2 = vector1.addVector(pitchAdjustedSinYaw * distance, sinPitch * distance, pitchAdjustedCosYaw * distance);
        return player.world.rayTraceBlocks(vector1, vector2); //from .worldObj --AstroTibs
    }
    ...
    public static void spawnExplosionParticleAtEntity(Entity entity)
    {
        for (int i = 0; i < 20; ++i)
        {
        	//Changed seven instances of .worldObj to .world --AstroTibs
