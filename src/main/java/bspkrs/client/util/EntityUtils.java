...
//Changed all instances of RendererLivingEntity to RenderEntity --AstroTibs
...
public class EntityUtils
{
	 ...
	 * mainModel out of the RenderEntity and try to render that to get
    ...
    public static float getModelSize(EntityLivingBase ent)
    {
        ...
        {
            RenderEntity entRender = (RenderEntity) render;
            ModelBase mainModel;
    ...
    public static void drawEntityOnScreen(int posX, int posY, float scale, float mouseX, float mouseY, EntityLivingBase ent)
    {
        ...
        try
        {
            ...
            rendermanager.doRenderEntity(ent, 0.0D, 0.0D, 0.0D, 0.0F, 1.0F, false); //idk what "false" does --AstroTibs
    ...
    /*
     * Look, man. I don't think the methods below are used for anything in particular. This is a core mod, after all.
     * --AstroTibs
     */
            ...
            List<SimpleEntry<UUID, String>> fallbackPlayerNames)
    {
        ...
        // Get a COPY dumbass!
        Set entities = new TreeSet(EntityList.NAME_TO_CLASS.keySet()); //changed from EntityList.stringToClassMapping -- AstroTibs
        ...
        do
        {
            ...
            clazz = (Class) EntityList.NAME_TO_CLASS.get(entStrings[id]); //changed from EntityList.stringToClassMapping -- AstroTibs
        }
