...
//import net.minecraft.client.renderer.VertexBuffer; // Changed all WorldRendered to VertexBuffer --AstroTibs
...
public final class HUDUtils
{
    ...
    public static void drawTexturedModalRect(int x, int y, int u, int v, int width, int height, float zLevel)
    {
        ...
        BufferBuilder worldRenderer = tessellator.getBuffer();//.getWorldRenderer(); --AstroTibs
        worldRenderer.begin(7, DefaultVertexFormats.POSITION);//.startDrawingQuads();
        /*
    ...
    public static void renderItemOverlayIntoGUI(FontRenderer fontRenderer, ItemStack itemStack, int x, int y, boolean showDamageBar, boolean showCount)
    {
        if ((itemStack != null) && (showDamageBar || showCount))
        {
            if (itemStack.isItemDamaged() && showDamageBar)
            {
                ...
                int[] var9 = {255 - var7, var7, 0, 255};  //int var9 = ((255 - var7) << 16) | (var7 << 8);
                int[] var10 = {(255 - var7) / 4, 64, 0, 255}; //int var10 = (((255 - var7) / 4) << 16) | 16128;
                renderQuad(var8, x + 2, y + 13, 13, 2, 0, 0, 0, 255);
                renderQuad(var8, x + 2, y + 13, 12, 1, var10[0], var10[1], var10[2], var10[3]);
                renderQuad(var8, x + 2, y + 13, var11, 1, var9[0], var9[1], var9[2], var9[3]);
    ...
    {
        int count = 0;
        for (int i = 0; i < player.inventory.mainInventory.size(); i++) //.length works for an Int[] --AstroTibs
        	//[i] only works for an Int[] so I've replaced it with .get(i) --AstroTibs
            if ((player.inventory.mainInventory.get(i) != null) && item.equals(player.inventory.mainInventory.get(i).getItem()) && ((md == -1) || (player.inventory.mainInventory.get(i).getMetadata() == md)))
                count += player.inventory.mainInventory.get(i).getCount();//[1].stackSize; --AstroTibs
        return count;
    }
